        ...
        >
            ...
            <div class="p-6 sm:p-8 overflow-y-auto">
                ...
                <div x-show="!isForgotPassword" x-cloak class="grid grid-cols-2 gap-4">
                    <button type="button" @click="window.location.href='{{ route('auth.google.redirect') }}';" class="flex items-center justify-center gap-2 py-3 px-4 border border-gray-200 hover:border-gray-300 hover:bg-gray-50 rounded-xl font-bold text-gray-700 text-sm transition-colors cursor-pointer focus:outline-none">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (forgotForm) {
        forgotForm.addEventListener('submit', function(e) { e.preventDefault(); /* ... */ });
    }
});
</script>
        ...
        loginForm.addEventListener('submit', function(e) {
            ...
            .then(({ ok, data }) => {
                ...
                window.dispatchEvent(new CustomEvent('show-auth-toast', {
                    detail: { message: data.message || 'Login gagal. Periksa email dan password Anda.', type: 'error' }
                }));
            })
            .catch(() => {
                window.dispatchEvent(new CustomEvent('show-auth-toast', {
        ...
        forgotForm.addEventListener('submit', function(e) {
            ...
            .then(({ ok, data }) => {
                ...
                if (ok && (data.success || data.message)) {
                    ...
                    window.dispatchEvent(new CustomEvent('show-auth-toast', { detail: { message: data.message || 'Gagal mengirim kode OTP', type: 'error' }}));
                }
            })
            .catch(() => {
                submitBtn.disabled = false;
